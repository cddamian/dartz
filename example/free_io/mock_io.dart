...
final EvaluationMonad<String,
    IMap<String, IVector<String>>,
    IVector<String>,
    IMap<String, int>> MockM = new EvaluationMonad(ivectorMi());
...
Evaluation<String,
    ...
    IVector<String>,
    IMap<String, int>,
    String?> mockReadFile(String fileName) =>
    MockM.gets((counters) => counters[fileName] | 0).bind((i) =>
        MockM.asks((inputs) => inputs[fileName] | emptyVector<String>()).bind((
            vs) =>
        // ignore: unnecessary_cast
        MockM.pure(vs[i].map((s) => s as String?) | null).bind((x) =>
            MockM.modify((counters) => counters.put(fileName, i + 1)).replace(
                x))));
...
Evaluation<String,
    ...
    IVector<String>,
    IMap<String, int>,
    A> _interpret<A>(Free<IOOp<dynamic>, A> op) =>
...
Evaluation<String,
    ...
    IVector<String>,
    IMap<String, int>,
    dynamic> mockIOInterpreter(IOOp<dynamic> io) {
  if (io is Readln) {
    return mockReadFile("stdin");
  } else if (io is Println) {
    return MockM.write(ivector(["stdout: ${io.s}"]));
  } else if (io is Attempt) {
    return _interpret(io.fa).map(io.succeed).handleError((e) =>
        MockM.pure(io.fail(e)));
  } else if (io is Fail) {
    ...
    return mockReadFile((io.file as _MockFileRef).name).map((s) =>
    s == null
        ? new UnmodifiableListView(<int>[])
        : new UnmodifiableListView(_utf8.encode(s)));
  } else if (io is WriteBytes) {
    return MockM.write(ivector([
      "${(io.file as _MockFileRef).name}: ${_utf8.decode(io.bytes.toList())}"
    ]));
  } else if (io is Execute) {
    return MockM.pure(new ExecutionResult(0,
        "<<< Mocked result of '${io.command} ${io.arguments.intercalate(
            StringMi, " ")}' >>>", ""));
  } else if (io is Delay) {
    return _interpret(io.a);
  } else if (io is Gather) {
    return io.ops.traverseEvaluation(ivectorMi<String>(), _interpret).map(
        io.cast);
  } else {
...
Future<Either<String,
    Tuple3<IVector<String>, IMap<String, int>, A>>> mockPerformIO<A>(
    Free<IOOp<dynamic>, A> io, IMap<String, IVector<String>> input) =>
    io.foldMapEvaluation(MockM, mockIOInterpreter).run(input, emptyMap());

Future<Either<String,
    Tuple3<IVector<String>, IMap<String, int>, IList<A>>>> mockConveyIO<A>(
    Conveyor<Free<IOOp<dynamic>, dynamic>, A> cio,
    IMap<String, IVector<String>> input) =>
