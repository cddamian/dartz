...
class ListTraversableMonadPlus extends Traversable<List<dynamic>> with Applicative<List<dynamic>>, ApplicativePlus<List<dynamic>>, Monad<List<dynamic>>, MonadPlus<List<dynamic>>, TraversableMonad<List<dynamic>>, TraversableMonadPlus<List<dynamic>>, Plus<List<dynamic>> {
...
final Traversable<List<dynamic>> ListTr = ListMP;

final Monoid<List<dynamic>> ListMi = new ListMonoid();
...
class ListTMonad<M> extends Functor<M> with  Applicative<M>, Monad<M> {
  ...
  Monad<dynamic> underlying() => ListMP;

  @override M pure<A>(A a) => _stackedM.pure([a]);
  M _concat(M a, M b) => _stackedM.bind(a, (Iterable<dynamic> l1) => _stackedM.map(b, (Iterable<dynamic> l2) => new List.from(l1)..addAll(l2)));
  @override M bind<A, B>(M mla, M f(A a)) => _stackedM.bind(mla, (List<dynamic> l) => ((l.isEmpty) ? pure([]) : l.map<M>(cast(f)).reduce(_concat)));
}
